...
import { ConcatenacionString, ConcatenarTraduccion } from "../Expresiones/ConcatenacionString";
...
export class Print implements Instruccion{
    ...
    traducir(ent: Entorno, arbol: AST,resultado3d:Resultado3D,temporales:Temporales) {
        let arrayConcatenacion = this.expresion.traducir(ent,arbol,resultado3d,temporales,0);
        
        if(this.expresion instanceof ConcatenacionString){
            arrayConcatenacion.forEach((CT:ConcatenarTraduccion)=>{
                let valAsign = CT.valAsign;
                let ultimoTipo = CT.tipo;

                if(ultimoTipo == Tipo.STRING){
